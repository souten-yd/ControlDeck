...
import { applicationBuilderApi, type ApplicationDesignProposal, type ApplicationPatchOperation, type ApplicationProject, type SemanticComponentCatalog } from "../../api/applicationBuilder";
...
export function DesignProposalGallery({ project, catalog, selectedComponentId, onReview, onClose }: {
  ...
  const [instruction, setInstruction] = useState("Workflowの入出力と動作を維持したまま、より使いやすいGUI構成を再検討してください");
  ...
  return <BottomSheet title="AI Design Proposals" onClose={onClose} wide stable><div className="space-y-4 pb-4">
    <div className="rounded-xl bg-violet-50 p-3 text-xs leading-relaxed text-violet-700 dark:bg-violet-950/30 dark:text-violet-300">自動構成済みのWorkflow契約と現在のGUIを選択したLLMへ渡し、Simple／Balanced／Denseの再検討案を生成します。自由codeではなく検証可能なApplication Spec Patchとして提示し、確認後だけ適用します。</div>
    ...
    {generate.data && <><div className="flex flex-wrap items-center gap-1"><span className="mr-auto text-xs font-semibold">Proposal viewport</span>{(["mobile", "tablet", "desktop"] as AppPreviewViewport[]).map((item) => <button key={item} type="button" onClick={() => setViewport(item)} aria-pressed={viewport === item} className={`min-h-11 rounded-lg px-2 text-[10px] capitalize ${viewport === item ? "bg-zinc-900 text-white dark:bg-white dark:text-zinc-900" : "bg-zinc-100 dark:bg-zinc-800"}`}>{item}</button>)}</div><section aria-label="Design proposals" className="grid gap-3 lg:grid-cols-3">{generate.data.proposals.map((proposal) => <ProposalCard key={proposal.id} proposal={proposal} catalog={catalog} viewport={viewport} onReview={() => onReview(proposal.patches)} />)}</section></>}
  </div></BottomSheet>;
}

function ProposalCard({ proposal, catalog, viewport, onReview }: { proposal: ApplicationDesignProposal; catalog: SemanticComponentCatalog; viewport: AppPreviewViewport; onReview: () => void }) {
  return <article className="flex min-w-0 flex-col rounded-2xl border border-zinc-200 p-4 dark:border-zinc-700"><span className="text-[10px] font-semibold uppercase tracking-wider text-violet-500">{proposal.direction}</span><h3 className="mt-1 text-sm font-semibold">{proposal.title}</h3><p className="mt-2 text-xs leading-relaxed text-zinc-500">{proposal.summary}</p><div className="my-3 min-w-0 overflow-hidden"><AppSpecPreview spec={proposal.preview.patchedSpec} catalog={catalog} viewport={viewport} compact label={`${proposal.direction} proposal`} /></div><p className="text-[10px] text-zinc-400">{proposal.patches.length} changes · {proposal.preview.valid ? "Valid" : "Needs review"}</p>{proposal.rationale.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-4 text-[10px] text-zinc-500">{proposal.rationale.map((item) => <li key={item}>{item}</li>)}</ul>}{proposal.preview.diagnostics.length > 0 && <p className="mt-2 text-[10px] text-amber-600">{proposal.preview.diagnostics[0].message}</p>}<button onClick={onReview} className="mt-auto min-h-11 rounded-xl border border-violet-300 px-3 text-xs font-semibold text-violet-700 dark:border-violet-800 dark:text-violet-300">Review this proposal</button></article>;
}
