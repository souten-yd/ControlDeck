...
import { applicationBuilderApi, type ApplicationPatchOperation, type ApplicationPatchPreview, type ApplicationProject, type SemanticComponentCatalog } from "../../api/applicationBuilder";
import { BottomSheet } from "../../components/ui";
import { useToasts } from "../../stores";
import { AppSpecPreview, type AppPreviewViewport } from "./AppSpecPreview";

export function ProposalDiffPanel({ project, catalog, onClose, initialOperations = [] }: { project: ApplicationProject; catalog: SemanticComponentCatalog; onClose: () => void; initialOperations?: ApplicationPatchOperation[] }) {
  ...
  const [viewport, setViewport] = useState<AppPreviewViewport>("mobile");
  ...
  return <BottomSheet title="Review Spec Patch" onClose={onClose} wide stable>
    <div className="space-y-4 pb-4">
      ...
      {preview && <section aria-label="Patch preview" className="space-y-3"><div className="grid grid-cols-2 gap-2"><SpecSummary title="Before" spec={project.spec} /><SpecSummary title="After" spec={preview.patchedSpec} /></div><section aria-label="Visual preview diff" className="space-y-2 rounded-xl border border-zinc-200 p-2 dark:border-zinc-700"><div className="flex flex-wrap items-center gap-1"><h3 className="mr-auto text-xs font-semibold">Visual comparison</h3>{(["mobile", "tablet", "desktop"] as AppPreviewViewport[]).map((item) => <button key={item} type="button" onClick={() => setViewport(item)} aria-pressed={viewport === item} className={`min-h-11 rounded-lg px-2 text-[10px] capitalize ${viewport === item ? "bg-zinc-900 text-white dark:bg-white dark:text-zinc-900" : "bg-zinc-100 dark:bg-zinc-800"}`}>{item}</button>)}</div><p className="text-[10px] leading-relaxed text-zinc-400">同じrendererとviewportで、保存中のSpecと選択Patch適用後を副作用なしに比較します。</p><div className="grid min-w-0 gap-2 lg:grid-cols-2"><div className="min-w-0 overflow-hidden"><span className="mb-1 block text-[10px] font-medium">Before</span><AppSpecPreview spec={project.spec} catalog={catalog} viewport={viewport} compact testId="visual-diff-before" label="Before" /></div><div className="min-w-0 overflow-hidden"><span className="mb-1 block text-[10px] font-medium">After</span><AppSpecPreview spec={preview.patchedSpec} catalog={catalog} viewport={viewport} compact testId="visual-diff-after" label="After" /></div></div></section>{preview.diagnostics.length > 0 && <div className="space-y-1">{preview.diagnostics.map((item, index) => <div key={`${item.code}-${index}`} className={`rounded-lg p-2 text-[10px] ${item.severity === "error" ? "bg-red-50 text-red-600 dark:bg-red-950/30 dark:text-red-300" : "bg-amber-50 text-amber-700 dark:bg-amber-950/30"}`}><strong>{item.code}</strong> · {item.message}<code className="mt-1 block break-all opacity-70">{item.path}</code></div>)}</div>}<p className="break-all font-mono text-[9px] text-zinc-400">base {preview.baseChecksum.slice(0, 12)}… → {preview.resultChecksum.slice(0, 12)}…</p></section>}
