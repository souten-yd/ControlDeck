...
export default function ApplicationFromWorkflowPage() {
  ...
  return <main className="min-h-0 flex-1 overflow-y-auto p-4 pb-24 md:p-6"><div className="mx-auto max-w-5xl">
    <PageHeader title="Workflowを自動でアプリ化" description="入出力を解析し、実行フォーム・結果表示・API接続まで動作するGUIを自動構成します。" leading={<button onClick={() => navigate(`/workflows/${workflowId}`)} aria-label="Back to Workflow" className="grid min-h-11 min-w-11 place-items-center rounded-xl text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900">←</button>} />
    <section className="my-5 rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900"><h2 className="text-sm font-semibold">ほぼ自動でアプリを作成</h2><p className="mt-1 text-xs leading-relaxed text-zinc-500">作成直後から生成可能な推奨GUIが入ります。必要な場合だけAIへ再検討を頼むか、Canvasで手動修正できます。</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><button disabled={create.isPending} onClick={() => create.mutate("draft")} className="min-h-11 rounded-xl bg-accent-600 text-sm font-semibold text-white disabled:opacity-50">現在のDraftを自動アプリ化</button><button disabled={create.isPending} onClick={() => create.mutate("published")} className="min-h-11 rounded-xl border border-zinc-300 text-sm font-medium dark:border-zinc-700">公開版を自動アプリ化</button></div></section>
