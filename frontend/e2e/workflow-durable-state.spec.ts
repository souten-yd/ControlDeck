...
function definition(operation: "set" | "increment") {
  return {
    nodes: [
      { id: "t", type: "trigger", name: "開始", config: { mode: "manual" }, position: { x: 40, y: 160 } },
      { id: "state", type: "data.state", name: "永続ステート", config: {
        operation, namespace: "metrics", key: "mobile_count",
        ...(operation === "set"
          ? { value: 1, value_type: "integer", expected_version: 0 }
          : { delta: 2 }),
      }, position: { x: 310, y: 160 } },
      { id: "out", type: "flow.return", name: "結果", config: {
        name: "result", renderer: "plain_text",
        value: "state-value={{state.value}}/version={{state.version}}/type={{state.value_type}}",
...
test("durable typed state is shared by published runs and configurable on mobile", async ({ page }) => {
  ...
  const workflowId = await page.evaluate(async (draft) => {
    ...
    const created = await fetch("/api/v1/workflows", {
      ...
      body: JSON.stringify({ name: "E2E Durable State", definition: draft }),
    });
  ...
  try {
    ...
    await expect(page.getByText("state-value=1/version=1/type=integer", { exact: true })).toBeVisible();
    ...
    await page.evaluate(async ({ id, draft }) => {
      ...
      if (!published.ok) throw new Error(await published.text());
    }, { id: workflowId, draft: definition("increment") });
    await page.reload();
    await page.getByRole("button", { name: "公開版を実行" }).click();
    await expect(page.getByText("state-value=3/version=2/type=integer", { exact: true })).toBeVisible();
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth - innerWidth)).toBeLessThanOrEqual(0);

    await page.setViewportSize({ width: 1280, height: 800 });
    await page.goto(`/workflows/${workflowId}`);
    await page.locator(".react-flow__node").filter({ hasText: "永続ステート" }).click();
    await expect(page.locator("#node-config-state-operation")).toHaveValue("increment");
    await expect(page.locator("#node-config-state-namespace")).toHaveValue("metrics");
    await expect(page.locator("#node-config-state-key")).toHaveValue("mobile_count");
    await expect(page.locator("#node-config-state-delta")).toHaveValue("2");
    await page.setViewportSize({ width: 320, height: 720 });
    await expect(page.locator("#node-config-state-operation")).toBeVisible();
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth - innerWidth)).toBeLessThanOrEqual(0);
