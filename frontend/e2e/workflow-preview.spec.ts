...
test("shows the exact publish blocker instead of a generic 409", async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole("menuitem", { name: "Preflight Check" }).click();
    ...
    await page.getByRole("button", { name: "More" }).click();
    await page.getByRole("menuitem", { name: "Publish Only" }).click();
...
test("integrates trigger input, safe preview, test result, and past input at 320px", async ({ page }) => {
  ...
  try {
    await page.goto(`/workflows/${workflowId}`);
    const testButton = page.getByRole("button", { name: "Test workflow draft without publishing" });
    const runButton = page.getByRole("button", { name: "Run workflow in editor; publish the saved draft when needed" });
    await expect(testButton).toBeVisible();
    await expect(runButton).toBeVisible();
    expect(await testButton.evaluate((test, run) => Boolean(test.compareDocumentPosition(run as Node) & Node.DOCUMENT_POSITION_FOLLOWING), await runButton.elementHandle())).toBe(true);
    await expect(testButton).toHaveAttribute("title", /公開版は変更しません/);
    await expect(runButton).toHaveAttribute("title", /公開してから実行/);
    const toolbarLayout = await page.evaluate(() => ({
      viewport: window.innerWidth,
      document: document.documentElement.scrollWidth,
      body: document.body.scrollWidth,
    }));
    expect(toolbarLayout.document).toBeLessThanOrEqual(toolbarLayout.viewport);
    expect(toolbarLayout.body).toBeLessThanOrEqual(toolbarLayout.viewport);

    await testButton.click();
    const preview = page.getByRole("complementary", { name: "確認・テスト" });
    await expect(preview).toBeVisible();
    await expect(preview.getByRole("radio", { name: /下書きをテスト/ })).toBeChecked();
    await preview.getByLabel("質問 *").fill("ControlDeck preview");
    await preview.getByRole("radio", { name: /実行前チェック/ }).click();
    await preview.getByRole("button", { name: "実行せず確認" }).click();
    await expect(preview.getByText("構造上は実行可能です")).toBeVisible();
    ...
    await page.setViewportSize({ width: 320, height: 700 });
    await page.getByRole("button", { name: "Run workflow in editor; publish the saved draft when needed" }).click();
    const runInputs = page.getByRole("dialog", { name: "実行時の入力" });
    await runInputs.getByLabel("質問 *").fill("ControlDeck editor run");
    await runInputs.getByRole("button", { name: "実行", exact: true }).click();
    await expect(page).toHaveURL(new RegExp(`/workflows/${workflowId}$`));
    await expect(page.getByLabel("実行デバッグパネル")).toBeVisible();
    await expect.poll(async () => page.evaluate(async (id) => {
      const response = await fetch(`/api/v1/workflow-executions?workflow_id=${id}`, { credentials: "same-origin" });
      const executions = await response.json() as Array<{ status: string }>;
      return executions[0]?.status;
    }, workflowId)).toBe("SUCCEEDED");
    await expect.poll(async () => page.evaluate(async (id) => {
