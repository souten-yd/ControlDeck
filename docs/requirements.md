...
- スマートフォンでも片手操作しやすい
- App Studioの主要導線は既存Workflowのアプリ化とする。利用者へ空のGUI設計を要求せず、Workflowの入出力契約から動作可能なフォーム、結果表示、実行接続を自動構成し、そのまま生成・動作確認へ進める。変更したい場合だけAIへの再検討指示またはCanvasでの任意修正を行い、AI案は差分検証後に選択適用する
...
- 共通Snippet libraryを持ち、名前・説明・code／prompt template・任意parameterを追加、編集、削除できる。複数Snippetは順序を明示して1回の実行へ組み合わせられる。実行前に対象、展開後内容、作業directory、条件をreviewする。
- 即時実行は独立したsystemd user serviceで行う`Detached run`を既定とし、再読込やSSH切断に耐える。既存tmuxへ入力する`Send to session`はprogram一致／shell待機などのpreconditionを持つ上級経路とし、条件不一致時は入力せず記録する。対話中sessionへ黙って文字列を注入しない。
- Scheduleは1回／毎日／毎週／隔週、timezone、次回時刻、期限超過時の追実行、実行timeout、preconditionを指定できる。Web process内timerではなくscheduleごとのsystemd user timerを生成し、再起動後も継続する。実行・skip・失敗・取消・Snippet変更／削除は監査し、bounded logと状態を表示する。
- UIはTerminal pageの共通`Snippets`入口と各sessionの3点menu内`Automation settings`へ集約する。PCはside panel、mobileはSafe Area対応bottom sheetとし、library／compose／scheduleを段階表示する。Terminal cardへ全操作を常時並べない。
...
- エディタの主操作は`Run`とし、未保存変更を保存してblockingがなければ現在draftを差分時だけ公開して実行し、同じエディタ内のdebug panelへ進捗と結果を表示する。変更がなければversionを増やさない。入力定義がある場合は実行前に入力dialogを表示する。
- 公開アプリは入力・進捗・承認・型付き出力・履歴だけを扱い、編集定義を返さない専用APIを使う。承認待ちはnode ID、redact済み承認文、担当者、ISO 8601期限を共通contractで返し、公開アプリ内の承認／却下で再開できる。公開アプリは`Workflows`一覧の`Open App`から開き、workflow IDを互換URL `/runner?workflow={id}`へ保持して再読込後も復元する。
- schedule、Webhook、system event、外部API、公開アプリはdraftを暗黙公開せず、明示済み公開版だけを実行する。ワークフロー一覧から既存公開版を開いても、編集中draftを更新しない。
- Workflow間の業務イベントは、発行側の`event.emit`と受信側のcustom event triggerをevent名で接続する。DB outboxへ先に保存し、明示的に公開・有効化した受信Workflowだけへat-least-once配送する。payloadはJSON object・64KiB以内、Secretは永続化前に伏字化し、本文を監査ログへ含めない。event ID、購読数、配送数、失敗数、受信実行IDを実行出力から確認でき、Webサービス再起動後も未完了配送を有限回再試行する。循環・過大fan-out・無限連鎖は有界に拒否する。
- 配列を同じサブWorkflowで処理する場合は`flow.map`を使い、公開版を開始時に1つ固定して全項目へ適用する。入力は型付きJSON array、各子へ`item`／`index`／`total`を渡し、最大100件・並列1〜5で実行して、完了順に依存せず入力順の結果、成功・失敗数、子execution IDを返す。失敗は親を停止するか型付きerrorとして収集するかを明示し、再帰cycleは子execution作成前に拒否する。Secret値は子へ渡せるが、親子の保存context、Map結果、監査metadataへ平文保存しない。
...
- グローバルnavigation、Quick Actions、command palette、機能ページ名は短い英語名へ統一する。Workflowの編集・公開実行入口は`Workflows`へ集約し、`workflows.edit`または`workflows.run`のいずれかを持つ利用者へ表示する。`workflows.run`だけの利用者には公開済み項目だけを返し、`/runner`は選択済み公開アプリの互換deep linkとして維持する。Application BuilderのUI製品名は`App Studio`とする。通常ページは共通PageHeaderを使い、titleを20px／28px、同じ余白・説明・action配置にする。AI Chat、Workflow Editor、接続中Terminal、Remote Viewer等の独立画面は対象外とする。
