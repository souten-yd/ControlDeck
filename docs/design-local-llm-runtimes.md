...
### サンプリング方針はモデル個別設定だけで決める

以前は「OpenCode が停止中のモデルを起こす場合は個別設定より優先して投機ONで常駐させる」
規則を持っていたが、実運用でエージェントが同じツール呼び出しを繰り返して止まらなくなった
ため外した。greedy は同じ文脈から同じ行動を決定的に出すので、エージェントループから
抜けられない（実測で毎ターン出力が59トークンで完全一致）。速度の利得より副作用が重い。

コーディング用途は llama.cpp へ寄せる。`draft-mtp` は temperature に依存せず投機が効く
（実測で temperature 0.0 と 0.7 に速度差なし）ので、サンプリングと速度を両立できる。

### 旧: OpenCode が起こす場合の扱い（廃止）
