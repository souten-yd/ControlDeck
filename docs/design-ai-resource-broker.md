...
時計での制限は置かない。振り回しを止めているのは「使用中なら退かない」の方で
あって、間隔ではない。誰も使っていない LLM を降ろす代償は、次に使う人が載せ直しを
待つこと（実測 80 秒、Qwen3.8-27B 23GB）だけで、**その人が居ないから降ろせている**。
間隔で縛ると、続けて来る生成が RAM 実行へ落ちる（実測で 17 倍遅い: 枠内 6.7 秒に
対し RAM のみ 113 秒）。
