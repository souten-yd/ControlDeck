...
- **安全プレビュー**: executor、secret 復号、外部通信、プロセス、DB 更新、ファイル書込を一切行わない静的解析。
- **テスト実行**: draft 定義とテスト入力を明示的に使う実実行。本番 scheduler / webhook / API 実行とは区別する。
...
| run API | saved definition を background task 実行 | `POST /workflows/{id}/run` | draft/published、idempotency、test mode がない | 互換 + API 追加 |
...
対応 node は success/error/timeout/no_data handle を宣言する。engine は node_id/type/message/code/retryable/attempt/redacted input summary/timestamp を持つ Error Context を route へ渡す。Authorization、cookie、password、token、API key、resolved secret は redact する。旧 `on_error=continue|branch|stop` は adapter で維持する。
...
Phase 3 は output.render、human.approval、control.merge/try/delay、data.template/filter/aggregate、flow.return/error/note、trigger.webhook/file/system/email boundary を優先する。`output.render`と`data.template/filter/aggregate`は実装済み。approval/merge/try/delay、flow制御、system trigger、typed error handleを次の機能単位とする。
