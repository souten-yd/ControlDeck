...
最終更新: 2026-07-20
状態: 実装仕様確定・Phase 1 UX 基盤、Phase 2 event replay／SSE・Alembic baseline、Phase 3 business event完了
...
- **更新して開く**: 未保存変更を保存し、公開blockingがなければ現在draftと同じ版を必要時だけ公開し、公開アプリ画面を開く主操作。公開済みで差分がなければ「アプリを開く」と表示する。
- **公開して直接実行**: エディタ上でノード状態まで観測したい高度なデバッグ操作。その他メニューに配置し、日常実行の主導線にはしない。
...
| stream | execution eventは認証SSE、切断時だけ低頻度fallback | `InfoPanel.tsx`, `workflows/events.py` | Preview等の残存pollingは別境界 | 実装済み／段階移行 |
...
| events | DB先行commitの単調sequence、replay API、SSE | `events.py`, engine/router | progress/log/artifact eventは後続 | コア実装済み |
| AI | schema 生成、semantic/quality、WS | `chat_router.py` | operation diff、diagnose、apply audit がない | patch service 追加 |
| migration | Alembic revision、起動時排他upgrade、既存SQLite backup／checksum／integrity／row count、head schema検証 | `database/migrations.py`, `alembic/` | legacy light migrationは移行期間だけ保持 | コア実装済み |
...
- `WorkflowPause`: execution/node/type/form schema/token hash/status/expires/resumed。平文 token は生成直後にhash化して破棄する。
- `WorkflowPinnedData`: draft/node/output/schema/user。publish payload へ含めない。
- `WorkflowExecutionEvent`: execution/sequence/type/node/timestamp/redacted payload。7日または1実行2,000件のTTL replay buffer、payload 64KiB、API batch 200件。
- `WorkflowArtifact`: storage key、filename、mime、size、checksum、sensitive、owner。DB blob は使わない。

Phase 2でAlembic baselineを導入済み。既存SQLiteは排他lock下でbackup → checksum／integrity／FK／全table件数検証 → baseline adoptを行い、以降はrevisionとmodel schemaを起動時に検証する。`create_all + light migration`は移行期間だけ互換処理として両立し、schema driftを補修して隠さない。
...
event は execution_id、node_id|null、sequence、timestamp、type が必須。実装済みのexecution.started/status/finished、node.started/finishedに続き、paused、progress/log/output/retry、artifact.createdを追加する。`Last-Event-ID`と`after_sequence`の大きい方から再接続し、保持範囲外はfull-state再取得を指示する。
...
Phase 3 は output.render、human.approval/form、control.merge/try/delay、data.template/filter/aggregate/queue/cache/state、event.emit、flow.return/error/note/map、trigger.webhook/file/system/email boundary を優先する。`output.render`、`human.approval`／`human.form`のDB-backed durable pause／JSON Schema入力／再起動継続、`control.merge`の5方式、`data.template/filter/aggregate`、typed error/timeout route、終端専用`flow.return`、非retryの`flow.error`／`test.assert`、副作用なし`flow.note`、durable `control.delay`、公開済みサブフローをsuccess/errorへ分ける`control.try`、GPU／VRAM／disk／llama-server／systemd／fileのsystem trigger、Workflow単位のdurable FIFO `data.queue`、TTL付き`data.cache`、期限なし型付き`data.state`、durable business event、version-pinned `flow.map`は実装済み。承認／Form／Delay待機はDB checkpointを共有し、editor／公開アプリ共通の型付きcontractと同一execution復元を使用する。`human.form`は最大20項目のtext／paragraph／number／boolean／select／multi-select／date／datetime／JSON object／JSON array／key-valueを扱い、Secret入力とfile uploadは受理しない。`data.queue`はenqueue／dequeue／peek／size、10,000件・1 item 256KiB上限、Secret永続化前redact、監査metadata、Workflow削除時清掃を持つ。`data.cache`はset／get／delete／size、1秒〜30日のTTL、Workflowあたり10,000 key・1 value 256KiB上限、期限切れのlazy清掃、Secret値redact・Secret key拒否、本文なし監査を持つ。`data.state`はget／set／delete／increment、初回値で固定するJSON型、単調version、expected version CAS、内部CAS retry、Workflowあたり10,000 key・1 value 256KiB上限、Secret安全性と本文なし監査を持ち、実行内`var.set`・memory-only client state・期限付きcacheと責務を分ける。`event.emit`は64KiB object payloadをDB outboxへ先行保存し、最新published definitionかつenabledの同名custom event triggerへat-least-once配送する。event ID、最大3回再送、100 subscriber、8 hop、lineage cycle拒否、10,000 event、7日保持を境界とし、Secretは保存前redact、監査には本文や例外本文を含めない。`flow.map`は最大100 item・並列1〜5で、開始時の同一published versionへitem／index／totalを型付き入力し、入力順の結果、成功・失敗数、子execution IDを返す。stop／collect失敗方針、3段depth上限に加えてlineage cycleを起動前に拒否し、Map監査にはpayloadを含めない。Tryの共通後処理はsuccess/errorをmergeし、暗黙finallyは持たない。schedule／Webhook／system eventは最新published definitionだけを選択・実行し、Webhook tokenは不可逆hashだけを公開版へ保持する。

次段階は batch／rate limit／circuit breaker、document/PDF/image/audio、unified notification。

差別化機能は execution time travel、VRAM/CTX/model 状態を使う `ai.route`、AI failure diagnose/patch、test-case batch regression を優先する。GPU/VRAM/disk/llama-server/systemd/file eventは既存triggerの`mode=system`として実装済みで、初回観測を抑止し変化時だけ発火する。sensor 取得不能は`N/A`／no_dataとしてイベントを捏造せず、アプリ全体を落とさない。
