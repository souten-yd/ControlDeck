...
将来の切分け診断ではresize要求/ACK/ACK後PTY出力/入力保留・解放、buffer type、cursor/base/viewport、cursor前後2行、表示中DOM行、
...
#### 世代付きPTY resize ACKと入力barrier（実機切分け後まで保留）

この節は、独自textarea同期と無条件focusを撤去したiPhone実機試験でも再現し、buffer側の二重行またはresize直後入力競合を
実測した場合だけStep 6として採用する詳細設計である。今回の緊急ロールバックには実装しない。

cols/rowsが実際に変わり、WebSocketへresizeを送る場合だけbarrierを開始する。

```text
term.resize(generation N)
  → resize JSON(N, connection generation)
...
100ms timeoutをフェイルセーフに使う。timerだけで順序問題を解決しない。通常入力、同一geometry、position-onlyではbarrierを作らない。
...
接続PTYへ`TIOCSWINSZ`を適用した結果をACKする。複数attach時はtmuxのclient/window寸法を診断値として確認するが、特定TUI名や
固定rowsを使った補正は行わない。
