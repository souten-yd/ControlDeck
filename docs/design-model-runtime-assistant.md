...
```json
{
  ...
  "max_loaded_models": 1,
  "amd_gpu_power": {"enabled": true, "watts": 210},
...
- `auto`: driverの動的制御（既定）。idle時の最低周波数と負荷時性能を両立する。
- `limit`: 選択level以下だけを許可し、発熱とLLM速度のバランスを取る。
- `minimum`: 最低levelだけを許可する静音優先。LLM帯域と応答速度が大幅低下し得ることを表示する。

手動level適用はdriverの仕様通りperformance levelを`manual`へ移してから行い、autoへ戻す操作も提供する。
...
`power_dpm_force_performance_level`、`pp_dpm_mclk`の3属性だけを書く。
