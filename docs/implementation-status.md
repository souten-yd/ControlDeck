...
2. ファイルの圧縮・解凍と PDF / 音声 / 動画等の高度プレビューは未実装
3. PostgreSQL の運用切替、汎用プラグインSDK、provider共通pull/設定管理は未完（OpenCode向けfeature境界は実装済み）
4. 電源 reboot/shutdown は API 実装済みだが、破壊的な実機実行は未検証
