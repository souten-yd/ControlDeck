...
| 文書整備 | ✅ 完了 |
| Phase 1 — 認証 + レイアウト | ✅ 完了 |
| Phase 2 — アプリ管理 | ✅ コア完了（アイコン対応済み。高度なヘルスチェックは残り） |
...
1. 電源の予約実行はプロセス内タイマー（Web 再起動で消失）。最小特権 helper + 永続 systemd timer が必要
2. system レベルの systemd サービス制御は未対応。helper / polkit の権限境界設計が必要
3. アプリ別 GPU 使用量と TCP / HTTP / 本文 / ファイル等の高度なヘルスチェックは未実装
4. ファイルの圧縮・解凍と PDF / 音声 / 動画等の高度プレビューは未実装
5. PostgreSQL の運用切替・プラグイン基盤・LLM runtime provider 抽象化・OpenCode は未完
6. 電源 reboot/shutdown は API 実装済みだが、破壊的な実機実行は未検証
