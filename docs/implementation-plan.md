...
- stdout / stderr・systemd journal・許可root内指定ファイルの秘密値マスキング済み表示 / ストリーム / ダウンロード / 削除
- Python 自動検出（候補提示のみ）
...
- ✅ アラート（CPU／RAM／GPU／VRAM／温度／disk／アプリ停止・HC失敗・再起動・ログERROR、Web通知センター + 暗号化メール／Discord／Slack／Webhook）
...
- ✅ TOTP policyを任意／管理者必須／全ユーザー必須から選択し、未設定時はenrollment API以外のREST／WebSocketを拒否。専用mobile／PC設定導線と必須時の無効化禁止を実装
- ◩ PostgreSQL運用切替: ✅ psycopg／0600 EnvironmentFile／疎通後切替・失敗rollback／Alembic advisory lock／offline head SQL／pg_dump・pg_restore。実PostgreSQL serverでの最終確認待ち
...
- Add-on Platform v2: ✅ PR-0 contract/harness、✅ PR-A backend registry/effective contributions（安全なmanaged copy、enable/grant、health/setup/partial availability、ETag/revision SSE、activity、audit）、✅ PR-B host-rendered UI contribution／状態表示（desktop sidebar、mobile More、Command Palette、v1/v2統合設定、権限／activity／setup、reload-free enable/disable）。次はPR-C embedded view/bridge、その後PR-D1 broker、単独migration、PR-D2 llama/Jobs、PR-E Workflow/Agentの順に進める
