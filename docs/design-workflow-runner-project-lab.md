# 公開アプリ（Workflow Runner）/ Project Lab 詳細実装仕様
...
公開済みワークフローを、編集キャンバスやノード定義を見せずに入力・承認・結果確認だけで運用できる「公開アプリ」を提供する。あわせて `~/CodeDEV` の開発成果物を自動検出し、CLI、Web、HTML、画像・グラフ、構造化データ、ログ、エラーを同じ画面で実行・評価する「Project Lab」を段階導入する。

公開アプリは編集画面の表示切替ではなく、公開版だけを読む専用APIをセキュリティ境界とする。Project LabはOpenCode optional featureに依存しないコア機能とし、OpenCodeは必要な場合だけAI修正・評価を行う追加能力として統合する。
...
ユーザー向けの最上位入口は`Workflows`へ統合する。`workflows.edit`利用者には編集一覧、`workflows.run`だけの利用者には公開済み一覧を表示する。各項目の`Open App`から、選択済み公開アプリだけを表示する互換route `/runner?workflow={id}`へ移る。workflow未指定の`/runner`は`/workflows`へ戻す。公開実行面にキャンバス、ノード名、実行経路は表示しない。

エディタは作成・確認・デバッグ、公開アプリは日常実行という責務を保つ。エディタの主操作`Run`は、差分があれば保存・公開検証・公開を行ってから実行し、差分がなければ公開versionを増やさず実行する。進捗と結果はエディタ内debug panelへ表示する。ワークフロー一覧の`Open App`は既存公開版だけを開き、編集中draftを暗黙更新しない。選択は `/runner?workflow={id}` に保持し、リロード後も復元する。
