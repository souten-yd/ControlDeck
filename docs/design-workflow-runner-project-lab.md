...
ユーザー向けページ名は「公開アプリ」、互換routeは `/runner`。左に公開アプリ一覧、右に入力・実行・結果を置き、390px以下では一覧→実行面の1カラム遷移とする。キャンバス、ノード名、実行経路は表示しない。

エディタは作成・確認・デバッグ、公開アプリは日常実行という責務を保つ。エディタの主操作は、差分があれば保存・公開検証・公開を行う「更新して開く」、差分がなければ「アプリを開く」とする。ワークフロー一覧は公開履歴のある項目から既存公開版を開けるが、下書きを暗黙更新しない。選択は `/runner?workflow={id}` に保持し、リロード後も復元する。
