...
- 通常回答とワークフロー生成は共通出力値を持たず、選択モデルのOllama `num_predict` / llama.cpp
  `n_predict`へ従う。無制限値は安全上限256Kへ正規化し、外部endpointだけ8,192へフォールバックする。
  context長と出力長は各モデル個別の別設定で、入力履歴と出力の合計がruntime context内に収まる必要がある。
