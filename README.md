...
### Project Lab（成果物previewと永続CLI/test実行）
...
`.controldeck/project.json`へ`cli`または`test` profileをargv配列で定義すると、Project Labから明示実行できる。実行はブラウザ接続から独立した制限付き`systemd --user` unitとなり、状態、終了code、経過時間、redact済みlog、実行後に生成・変更されたartifactを同じ履歴で確認・停止できる。Secret参照profile、shell文字列、project外cwd、未許可binaryは安全なcredential/adapterが実装されるまで実行しない。
