...
9. 「検証して実行」を押す。保存と公開前検証を行い、draftに差分がある場合だけ新しい公開版へ切り替えて、その固定版を実行する。実行せず配備だけ更新する場合は「その他 → 実行せず公開」を使う。
...
実行前チェックとdraftテストの両方で「構造上は実行可能」と「公開できます」を分けて表示する。「検証して実行」も同じ判定を使い、
